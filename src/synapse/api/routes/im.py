...
def _get_storage():
    try:
        from synapse.config import settings
        from synapse.memory.storage import get_shared_storage

        return get_shared_storage(settings.project_root / "data" / "memory" / "synapse.db")
    except Exception:
        return None
...
async def delete_im_session(request: Request, session_id: str):
    ...
    try:
        ...
        db_path = settings.project_root / "data" / "memory" / "synapse.db"
...
async def set_bot_config(request: Request, body: BotConfigRequest):
    ...
    from synapse.channels.bot_config import BotConfigRule
...
async def set_group_policy(request: Request, body: GroupPolicyRequest):
    ...
    from synapse.channels.group_response import GroupResponseMode
