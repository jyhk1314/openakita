...
本文档详细介绍 Synapse 的人格系统（Persona System）和活人感引擎（Proactive Engine），这两个特性让 Agent 不再是冷冰冰的工具，而是一个有温度、有个性的智能助理。
...
以下配置可在 `.env` 文件、Setup Center 或 `synapse init` 向导中设置：
