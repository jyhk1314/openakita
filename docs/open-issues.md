...
- **问题**: 调用 `browser_open()` 时 Playwright 报 Chromium 可执行文件不存在，路径 `~\.synapse\modules\browser\browsers\chromium-1208\chrome-win64\chrome.exe` 不存在。Agent 尝试自动执行 `playwright install` 但过程被中止。
...
- **涉及代码**: `src/synapse/tools/browser_mcp.py`（浏览器启动逻辑）、模块安装脚本
...
- **涉及代码**: `apps/setup-center/src/App.tsx`（skill 状态切换 UI）、`src/synapse/skills/registry.py`（运行时加载）
...
- **涉及代码**: `src/synapse/llm/providers/base.py`（端点错误处理 / failover 逻辑）
...
- **涉及代码**: `src/synapse/llm/providers/openai.py`（请求构建）、`src/synapse/config/`（配置定义）、`apps/setup-center/src/App.tsx`（UI 配置项）
...
- **涉及代码**: `src/synapse/channels/adapters/telegram.py`（消息过滤逻辑）、`src/synapse/config/`（新增配置项）
