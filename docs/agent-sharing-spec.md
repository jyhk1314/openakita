...
本规范定义了 Synapse Agent 包（`.akita-agent`）的格式标准，用于在用户之间共享完整的 AI Agent 配置，包括提示词、技能和元数据。
...
| `min_platform_version` | string | 否 | 要求的最低 Synapse 版本 |
...
Agent 运行时配置，与 Synapse 的 `AgentProfile` 数据结构对应：
...
捆绑技能遵循标准的 Synapse Skill 格式。每个子目录代表一个技能，必须包含 `SKILL.md` 文件。
