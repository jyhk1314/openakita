...
本文档全面梳理 Synapse 项目的提示词系统架构，包括静态提示词文件、动态生成逻辑、以及各组件的协作关系。
...
Synapse 是一个全能自进化AI助手，核心目标是成为一个真正对用户有帮助的助手。
...
**位置**: `src/synapse/core/agent.py`

构建最终的系统提示词（**v2：编译管线 + 预算裁剪 + 显式分段**）。

当前实现的“最终 system prompt”主要由 `Prompt Builder` 负责组装，而不是把各段直接字符串拼接在 `Agent._build_system_prompt()` 里：

- **Prompt Builder**：`src/synapse/prompt/builder.py::build_system_prompt()`
- **预算裁剪**：`src/synapse/prompt/budget.py`
- **记忆检索**：`src/synapse/prompt/retriever.py::retrieve_memory()`
...
**位置**: `src/synapse/core/agent.py` 中的 `PROMPT_COMPILER_SYSTEM`
...
**位置**: `src/synapse/memory/extractor.py`
...
**位置**: `src/synapse/core/brain.py` - `plan()`
...
**位置**: `src/synapse/core/brain.py` - `generate_code()`
...
**位置**: `src/synapse/core/brain.py` - `analyze_error()`
...
**位置**: `src/synapse/core/agent.py` - `BASE_TOOLS`
...
| `src/synapse/core/identity.py` | 代码 | 身份管理，提示词生成 |
| `src/synapse/core/agent.py` | 代码 | 主类，动态提示词组装 |
| `src/synapse/core/brain.py` | 代码 | LLM 交互，内置提示词 |
| `src/synapse/memory/extractor.py` | 代码 | 记忆提取提示词 |
