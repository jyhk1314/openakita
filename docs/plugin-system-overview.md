# Synapse 插件系统概览：Skill vs MCP vs Plugin

本文档说明 Synapse 三种扩展机制的定位、能力边界和选型指南。
...
MCP (Model Context Protocol) 是独立进程的外部工具服务，通过 JSON-RPC 协议与 Synapse 通信。

- **能做什么**：提供工具（tools），语言无关（Node.js / Python / Go 等）
- **不能做什么**：不能注入 prompt、不能挂钩子、不能注册 IM 通道、不能替换记忆、不能访问 Synapse 内部服务
...
插件是 Synapse 的新一代扩展机制，通过 `PluginAPI` 与宿主进行受控交互，可以提供 8 种能力。
...
- 需要访问 Synapse 内部服务（记忆、Brain、消息网关）
...
| 版本类型 | 当前值 | 说明 |
|----------|--------|------|
| System Version | `1.27.7` | Synapse 整体发布版本号 |
...
实现文件：`src/synapse/plugins/compat.py`
