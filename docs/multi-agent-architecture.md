...
本文档描述 Synapse 的多 Agent 架构设计。该架构通过 `multi_agent_enabled` 开关与单 Agent 模式完全隔离，标记为 **Beta**。
...
```
src/synapse/agents/fallback.py
```
...
```
src/synapse/agents/task_queue.py
```
...
```
src/synapse/agents/lock_manager.py
```
...
**注意事项：**
- `SYNAPSE_ROOT` 环境变量可覆盖 `synapse_home`，不影响 `data_dir`
- `skills_path` 在 `user_workspace_path/skills` 下，技能全局共享
- Agent profiles 是 per-workspace 的，但系统预置会在每个工作区首次启用时自动部署

## 旧架构废弃说明

`synapse.orchestration` 模块（基于 ZMQ 的 Master-Worker 架构）已标记为 `@deprecated`：

- 所有公共类（`AgentBus`, `MasterAgent`, `WorkerAgent`）在 `__init__` 中发出 `DeprecationWarning`
- `pyzmq` 已从核心依赖移至可选依赖：`pip install synapse[orchestration]`
- 代码未删除，但不再维护
- 新功能请使用 `synapse.agents` 包
