...
> 调研日期: 2026-02-02  
> 调研目的: 了解各主流 LLM 模型在工具调用、Thinking 模式、多模态传输等方面的接口差异，为 Synapse 多模型适配提供参考
...
**系统处理方案** (已实现):
- 文件: `src/synapse/llm/converters/tools.py`
- 函数: `parse_text_tool_calls()`, `has_text_tool_calls()`
- 在 `src/synapse/llm/providers/anthropic.py` 的 `_parse_response()` 中自动检测和解析
...
- [Synapse LLM Tool Call Formats](./llm-tool-call-formats.md) - 工具调用格式快速参考
