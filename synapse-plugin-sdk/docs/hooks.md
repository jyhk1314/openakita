...
Synapse 提供 **10 个钩子**，覆盖插件的完整生命周期：初始化、消息收发、记忆检索、工具调用、会话管理、Prompt 构建和定时任务。

Synapse provides **10 hooks** covering the full plugin lifecycle: initialization, message I/O, memory retrieval, tool execution, session management, prompt assembly, and scheduled tasks.
...
```python
from synapse_plugin_sdk.hooks import HOOK_SIGNATURES
