...
```python
from synapse_plugin_sdk import PluginAPI
```
...
```python
from synapse_plugin_sdk.channel import ChannelAdapter
...
```python
from synapse_plugin_sdk.llm import LLMProvider, ProviderRegistry, ProviderRegistryInfo
...
|-------------|--------------|----------------|
| `PluginBase` | `synapse_plugin_sdk` | 插件入口基类 / Plugin entry class |
| `PluginManifest` | `synapse_plugin_sdk` | 清单数据类 / Manifest dataclass |
| `tool_definition()` | `synapse_plugin_sdk.tools` | 工具定义构建器 / Tool definition builder |
| `ToolHandler` | `synapse_plugin_sdk.tools` | 工具处理基类 / Tool handler base |
| `ChannelAdapter` | `synapse_plugin_sdk.channel` | 通道适配器基类 / Channel adapter base |
| `LLMProvider` | `synapse_plugin_sdk.llm` | LLM 提供商基类 / LLM provider base |
| `MemoryBackendProtocol` | `synapse_plugin_sdk.protocols` | 记忆后端协议 / Memory backend protocol |
| `RetrievalSource` | `synapse_plugin_sdk.protocols` | 检索源协议 / Retrieval source protocol |
| `SearchBackend` | `synapse_plugin_sdk.protocols` | 搜索后端协议 / Search backend protocol |
| `MockPluginAPI` | `synapse_plugin_sdk.testing` | 测试用模拟 / Test mock |
| `HOOK_NAMES` | `synapse_plugin_sdk.hooks` | 有效钩子名集合 / Valid hook names |
| `HOOK_SIGNATURES` | `synapse_plugin_sdk.hooks` | 钩子回调签名参考 / Hook callback signature reference |
| `UnifiedMessage` | `synapse_plugin_sdk.types` | 统一消息类型 / Unified message type |
