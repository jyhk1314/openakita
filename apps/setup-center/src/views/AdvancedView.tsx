...
export function AdvancedView(props: AdvancedViewProps) {
    ...
    if (apiUrl) {
      ...
      safeFetch(`${apiUrl}/api/system-info`, { signal: AbortSignal.timeout(8_000) })
        ...
        .then((data) => {
          ...
          if (data.synapse_version) sysInfo["Backend"] = data.synapse_version;
      ...
      safeFetch(`${apiUrl}/api/config/env`, { signal: AbortSignal.timeout(5_000) })
        ...
        .then((data) => {
          ...
          if (data.env?.HUB_ENABLED != null) {
            ...
            localStorage.setItem("synapse_storeVisible", String(enabled));
          }
        })
        .catch(() => {});
    }
  ...
  async function fetchSystemInfo() {
    ...
    try {
      ...
      if (data.synapse_version) sysInfo[t("adv.sysVersion")] = data.synapse_version;
      setAdvSysInfo(sysInfo);
        ...
        <Section title={t("adv.hubTitle")}
          toggle={
            <Switch
              ...
              onCheckedChange={(v) => {
                setStoreVisible(v);
                localStorage.setItem("synapse_storeVisible", String(v));
                if (shouldUseHttpApi()) {
