...
@test("Tracing 框架基本功能")
def _():
    from synapse.tracing.tracer import AgentTracer, SpanType, SpanStatus, get_tracer, set_tracer
...
def _():
    from synapse.tracing.exporter import FileExporter, ConsoleExporter, TraceExporter
...
def _():
    from synapse.core.response_handler import (
...
def _():
    from synapse.core.skill_manager import SkillManager
    assert SkillManager is not None


@test("PromptAssembler 导入")
def _():
    from synapse.core.prompt_assembler import PromptAssembler
    assert PromptAssembler is not None


@test("ReasoningEngine 和 Checkpoint")
def _():
    from synapse.core.reasoning_engine import (
...
def _():
    from synapse.tools.catalog import HIGH_FREQ_TOOLS, ToolCatalog
...
@test("评估框架 - Metrics")
def _():
    from synapse.evaluation.metrics import EvalMetrics, EvalResult, TraceMetrics
...
@test("评估框架 - Judge")
def _():
    from synapse.evaluation.judge import Judge, JudgeResult
...
@test("评估框架 - Optimizer")
def _():
    from synapse.evaluation.optimizer import (
...
def _():
    ...
    # Phase 1
    from synapse.core.agent_state import AgentState, TaskState, TaskStatus
    from synapse.tracing import AgentTracer, Span, SpanType, SpanStatus, Trace, get_tracer, set_tracer
    from synapse.tracing.exporter import FileExporter, ConsoleExporter
    from synapse.tools.errors import ToolError, ErrorType, classify_error

    # Phase 2
    from synapse.core.tool_executor import ToolExecutor
    from synapse.core.context_manager import ContextManager
    from synapse.core.response_handler import ResponseHandler
    from synapse.core.skill_manager import SkillManager
    from synapse.core.prompt_assembler import PromptAssembler
    from synapse.core.reasoning_engine import ReasoningEngine, Checkpoint, Decision
    ...
    from synapse.evaluation.metrics import EvalMetrics, EvalResult, TraceMetrics
    from synapse.evaluation.judge import Judge, JudgeResult
    from synapse.evaluation.runner import EvalRunner
    from synapse.evaluation.reporter import Reporter
    from synapse.evaluation.optimizer import FeedbackAnalyzer, FeedbackOptimizer, DailyEvaluator
