...
from synapse.core.intent_analyzer import (
    IntentAnalyzer,
    IntentResult,
    IntentType,
...
from synapse.prompt.budget import BudgetConfig
from synapse.prompt.compiler import compile_all, check_compiled_outdated
...
class TestPromptCompilerNoTooling:
    ...
    def test_no_compile_agent_tooling_function(self):
        from synapse.prompt import compiler
...
class TestIdentityRouteNoTooling:
    def test_budget_map_no_tooling(self):
        from synapse.api.routes.identity import _BUDGET_MAP
        assert "runtime/agent.tooling.md" not in _BUDGET_MAP

    def test_runtime_files_no_tooling(self):
        from synapse.api.routes.identity import _RUNTIME_FILES
