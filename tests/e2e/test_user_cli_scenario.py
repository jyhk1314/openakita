...
class TestCLIStartup:
    """User runs `synapse` or `python -m synapse` for the first time."""

    def test_module_importable(self):
        """The package can be imported without errors."""
        result = subprocess.run(
            [sys.executable, "-c", "import synapse; print('OK')"],
    ...
    def test_help_flag(self):
        ...
        combined = result.stdout + result.stderr
        assert "usage" in combined.lower() or "help" in combined.lower() or "synapse" in combined.lower()
...
class TestCLISubcommands:
    ...
    def test_status_command_exists(self):
        """'synapse status' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_init_command_exists(self):
        """'synapse init' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_selfcheck_command_exists(self):
        """'synapse selfcheck' should be a recognized command."""
        result = subprocess.run(
    ...
    def test_serve_command_exists(self):
        """'synapse serve --help' should show API server options."""
        result = subprocess.run(
            [sys.executable, "-m", "synapse", "serve", "--help"],
            capture_output=True, text=True, timeout=15,
        )
        combined = result.stdout + result.stderr
