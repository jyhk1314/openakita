# Synapse AI 探索性全面自测报告（2026-04-03）
...
若需追踪 **图片 SSE error** 根因，请在复现时在同一时间段检索 **Synapse 进程标准输出 / 日志文件**（具体路径取决于 `synapse serve` 启动方式与 `LOG_*` 配置），并与 `llm_request_*` 时间戳 `20260403_1214xx` 对齐。
